...
lines.forEach((line) => {
  ...
  const springsState = [
    ...Array.from(springsStatePartial), '?', 
    ...Array.from(springsStatePartial), '?',  
    ...Array.from(springsStatePartial), '?', 
    ...Array.from(springsStatePartial), '?', 
    ...Array.from(springsStatePartial)
  ]
  const damagedGroupsSizes = damagedGroupsSizesPartial
    .concat(',', damagedGroupsSizesPartial)
    .concat(',', damagedGroupsSizesPartial)
    .concat(',', damagedGroupsSizesPartial) 
    .concat(',', damagedGroupsSizesPartial) 
    .split(',').map(size => Number(size))

  console.log(springsState, damagedGroupsSizes)
